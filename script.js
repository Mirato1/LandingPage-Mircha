...
import { TargetName } from './components/TargetName.js';
	...
	const cb = (entries) => {
		entries.forEach((entry) => {
			...
			if (entry.isIntersecting) {
				d.querySelector(`a[data__scroll--spy][href="#${id}"]`).classList.add('spy__on');
			} else {
				d.querySelector(`a[data__scroll--spy][href="#${id}"]`).classList.remove('spy__on');
			}
		});
	};
